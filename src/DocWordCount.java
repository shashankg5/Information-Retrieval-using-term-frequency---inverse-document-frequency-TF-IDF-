   ...
   public static class Map extends Mapper<LongWritable ,  Text ,  Text ,  IntWritable > {
      ...
      public void map( LongWritable offset,  Text lineText,  Context context)
        throws  IOException,  InterruptedException {
         ...
         String line  = lineText.toString().toLowerCase();	//Converting to lower case "IBM" equals "ibm"
         Text currentWord  = new Text();
		 String filename = ((FileSplit)context.getInputSplit()).getPath().getName(); //Getting the filename

         for ( String word  : WORD_BOUNDARY .split(line)) {
            if (word.isEmpty()) {
               continue;
            }
            currentWord  = new Text(word+"#####"+filename); //Creating keys with the filename
